...
import { getProfile, getImagePicker, getIMG, updatePlan, uploadImg, getFireBaseRef, resetHistory, resetPlayer } from '../screens/helper'
...
export const OnlinePlayer = {
  store: makeAutoObservable<OnlinePlayerStore>({
      start: false,
      finish: false,
      stepTime: 0,
      timeText: ' ',
    ...
    try {
      ...
      if (curProf) {
        OnlinePlayer.store = {
            ...
            history: curProf.history.sort(
                (a, b) => b.createDate - a.createDate).slice(0, 30)
        }
