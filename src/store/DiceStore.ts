...
autorun(() => {
  ...
  if (i18next.isInitialized) {
    ...
    if (DiceStore.online) {
      if (!isReported) {
        textTopMess = i18next.t('online-part.notReported')
      } else if (canGo) {
        textTopMess = i18next.t('takeStep')
      } else {
        textTopMess = `${i18next.t('nextStep')}: ${timeText}`
      }
    } else {
      textTopMess = `${i18next.t('playerTurn')} # ${DiceStore.players}`
    }
