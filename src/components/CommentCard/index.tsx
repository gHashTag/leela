...
export const CommentCard: React.FC<CommentCardI> = ({
    ...
    return <View style={container}>
        <View style={{marginRight: s(6)}}>
            <PlanAvatar plan={23} size='medium' />
            {endIndex !== index && <View style={{flex: 1, alignItems: 'center' }} onLayout={_onLayout}>
                <View style={[verticalLine, {height: lineHeight + PADDING*2 - vs(4)}]} />
            </View>}
        </View>
        <View style={{ flexDirection: 'column', flex: 1 }}>
            <View style={{ flexDirection: 'row' }} >
                <Txt textAlign="left" h6 title={`${item.firstName}`} />
                <Txt textStyle={{color: lightGray}} textAlign="left" h6 title={` ·${date}`} />
            </View>
            <Txt title={`${item.text}`} />
        </View>
    </View>
