...
const routeLabels: Record<string, string> = {
  // The 3D board is what the app opens on; the flat one moved to slot 3.
  // These labels are a second map of the same routes, so a screen swapped in
  // `Navigation.tsx` and not swapped here names the wrong thing.
  TAB_BOTTOM_0: 'tabRoute.board',
  TAB_BOTTOM_1: 'tabRoute.feed',
  TAB_BOTTOM_2: 'tabRoute.profile',
  TAB_BOTTOM_3: 'tabRoute.game',
    ...
    >
      {routes.map(({ name, key }, id) => {
        ...
        const hint = isFocused
          ? t('tabRoute.activeHint')
          : t('tabRoute.inactiveHint')
        return (
          <Pressable
            key={key}
