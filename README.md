...
| `@leela/ai` | 292 | the companion — prompts built from the plan text |
| `@leela/contracts` | 95 | `LeelaGame.sol`, board verified against the engine — [readme](packages/contracts/README.md) |
| `@leela/bot` | 1206 | group play in Telegram, durable on SQLite — [readme](apps/bot/README.md) |
...
| `@leela/webgl` | 620 | the board in three dimensions, in a browser, on the same rules the apps play |
| everything else | — | not yet ported |

5124 tests, run on every push by [CI](.github/workflows/ci.yml), which also
