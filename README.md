...
| `@leela/content` | 717 | 22 languages of plans, 2 of the game's own voice |
...
| `@leela/ai` | 249 | the companion — prompts built from the plan text |
| `@leela/contracts` | 95 | `LeelaGame.sol`, board verified against the engine — [readme](packages/contracts/README.md) |
| `@leela/bot` | 1152 | group play in Telegram, durable on SQLite — [readme](apps/bot/README.md) |
...
| `@leela/webgl` | 612 | the board in three dimensions, in a browser, on the same rules the apps play |
| everything else | — | not yet ported |

5019 tests, run on every push by [CI](.github/workflows/ci.yml), which also
