...
| Package | Tests | State |
|---|---|---|
| `@leela/engine` | 570 | rules, four variants, sessions, turn gating, seeded dice |
| `@leela/content` | 712 | 22 languages of plans, 2 of the game's own voice |
...
| `@leela/bot` | 999 | group play in Telegram, durable on SQLite — [readme](apps/bot/README.md) |
| `@leela/docs` | 239 | the book, live at [t27.ai/leela/docs](https://t27.ai/leela/docs/) — [readme](apps/docs/README.md) |
| `@leela/miniapp` | 711 | the board as a mini app, live at [t27.ai/leela](https://t27.ai/leela/) — [readme](apps/miniapp/README.md) |
| `@leela/mobile` | 421 | the board on a phone (Expo), moved by the engine and by nothing else |
| `@leela/webgl` | 592 | the board in three dimensions, in a browser, on the same rules the apps play |
| everything else | — | not yet ported |

4820 tests, run on every push by [CI](.github/workflows/ci.yml), which also
...
node scripts/audit-namesakes.mjs    # no language calls two plans by one name that is not
                                    # already written down, with why it is left alone
node scripts/audit-prose.mjs        # the shipped text is prose, not the markup it was
                                    # carried in, and no plan stops mid-sentence
```
