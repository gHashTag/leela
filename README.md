...
| Package | Tests | State |
|---|---|---|
| `@leela/engine` | 548 | rules, four variants, sessions, turn gating, seeded dice |
...
| `@leela/miniapp` | 584 | the board as a mini app, live at [t27.ai/leela](https://t27.ai/leela/) — [readme](apps/miniapp/README.md) |
| `@leela/mobile` | 408 | the board on a phone (Expo), moved by the engine and by nothing else |
| `@leela/webgl` | 550 | the board in three dimensions, in a browser, on the same rules the apps play |
| everything else | — | not yet ported |

4532 tests, run on every push by [CI](.github/workflows/ci.yml), which also
